...
<body>
                            ...
                            contentHtml += `
                                <div class="col-lg-4 col-md-6">
                                    <div class="balance-card">
                                        ...
                                        <p class="balance-value">${parseFloat(balance.remaining_balance).toFixed(2)} <span class="balance-unit">يوم</span></p>
                                        <p class="balance-details">
                                            المتاح: ${parseFloat(balance.balance_allotted).toFixed(2)} | 
                                            المستخدم: ${parseFloat(balance.balance_consumed).toFixed(2)}
                                        </p>
                                    </div>
                                </div>`;
                        });
    ...
    showLoading();
    try {
        const response = await fetch('<?= site_url("users1/check_last_payslip_status") ?>');
        const result = await response.json();
        
        if (result.status === 'success') {
            let noteHtml = '';
            if (result.note) {
                noteHtml = `<div class="alert alert-info mt-3">${result.note}</div>`;
            }
                    ...
                    <div class="col-md-8">
                        <div class="card shadow-sm border-0">
                            <div class="card-body p-5">
                                <div class="mb-4">
                                    <i class="fas fa-file-invoice-dollar fa-5x text-success"></i>
                                </div>
                                <h3 class="mb-3" style="font-family: 'El Messiri', sans-serif;">مسير رواتب شهر: <span dir="ltr"> مارس</span></h3>
                                <p class="text-muted mb-4">تم إصدار واعتماد مسير الرواتب لهذا الشهر.</p>
                                
                                <div class="d-grid gap-2 col-md-8 mx-auto">
                                    <a href="${result.download_url}" class="btn btn-primary btn-lg" target="_blank">
                                        <i class="fas fa-print me-2"></i> طباعة / عرض التعريف
                                    </a>
                                </div>
                                ${noteHtml}
                            </div>
            ...
            let currentMonth = new Date().toLocaleString('ar-SA', { year: 'numeric', month: 'long' });
            detailsContainer.innerHTML = `
                <div class="text-center mt-5">
                    <i class="fas fa-clock fa-4x text-muted mb-3"></i>
                    <h3>لم يتم الإصدار بعد</h3>
                    <p class="text-muted">مسير راتب شهر ${currentMonth} لم يتم اعتماده حتى الآن.</p>
                    <p class="text-muted small">سيظهر هنا بمجرد اعتماده من قبل إدارة الموارد البشرية.</p>
                </div>`;
        }
    } catch (error) {
        detailsContainer.innerHTML = `<div class="alert alert-danger">حدث خطأ في الاتصال.</div>`;
    }
}
