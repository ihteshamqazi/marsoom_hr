<?php /* application/views/templateo/employee_eval_a4_v3.php */ ?>
<!DOCTYPE html>
...
<div class="no-print">
  ...
  <a class="btn" href="<?php echo site_url('users/user_report101'); ?>">شاشة التقييم</a>
...
<div class="a4-sheet">
  ...
  <!-- بطاقات ملخص سريعة -->
  <?php
    // الاعتماد الآن فقط على الإجمالي بدون أي معيار استثنائي
    $total = (float)($ev_done['w15'] ?? 0);
  ?>
  <div class="summary">
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="value">
        <span class="big-badge">% <?php echo number_format($total, 2); ?></span>
      </div>
    </div>
  </div>
  ...
  <table class="print-table">
    ...
    <tbody>
      <tr>
        <td class="center"><?php echo htmlspecialchars($ev_done['year'] ?? '2024'); ?></td>
  ...
  <!-- معايير التقييم (بدون المعيار الاستثنائي) -->
  <?php
    $p_prod   = (float)($ev_done['w13'] ?? 0); // الإنتاجية 25%
    $p_lead   = (float)($ev_done['w22'] ?? 0); // المهارات القيادية 15%
    $p_tasks  = (float)($ev_done['w21'] ?? 0); // المهام 25%
    $p_behav  = (float)($ev_done['w9']  ?? 0); // السلوك 15%
    $p_attend = (float)($ev_done['w14'] ?? 0); // الحضور 20%
  ?>
  ...
  <table class="print-table">
    <thead>
      <tr>
        <th>الإنتاجية 50%</th>
        <th>المهارات القيادية 10%</th>
        <th>المهام الوظيفية 15%</th>
        <th>السلوك العام 5%</th>
        <th>الحضور والانصراف 20%</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td class="center">% <?php echo number_format($p_prod, 2);   ?></td>
        <td class="center">% <?php echo number_format($p_lead, 2);   ?></td>
        <td class="center">% <?php echo number_format($p_tasks, 2);  ?></td>
        <td class="center">% <?php echo number_format($p_behav, 2);  ?></td>
    ...
    <tfoot>
      <tr>
        ...
        <th colspan="3" class="center">% <?php echo number_format($total, 2); ?></th>
      </tr>
    </tfoot>
  </table>
...
<br/>
<br/>
    <div style="font-size:12px;">للاعتراض على التقييم نامل تحميل التقييم وارسال الاعتراض عن طريق البريد الإلكتروني  إلى الادارة المعنية وشكرا   </div>
  </div>
  <?php endif; ?>
