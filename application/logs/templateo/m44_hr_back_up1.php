...
for ($date = $start_date; $date <= $end_date; $date = strtotime("+1 day", $date)) {
  ...
  if ($is_on_vacation) {
    ...
    elseif (empty($first_punch) || empty($last_punch) || (($last_punch - $first_punch) < 60)) {
      ...
      // نقرأ first_punch و last_punch من القيود
      ...
      // --- START MODIFICATION TO IGNORE SECONDS ---
      // تسوية كل الأوقات بإزالة الثواني منها (جعلها 00) قبل الحساب
      ...
      // البداية الرسمية: first_punch إن وُجد، وإلا last_punch، وإلا 08:00
      ...
      // حد الدخول الأقصى (الساعات المرنة) من last_punch، وإن لم يوجد فهو = بداية الدوام
      $max_entry = ($rule_row && !empty($rule_row->last_punch))
        ? strtotime(date('Y-m-d H:i:00', strtotime($date_str . ' ' . $rule_row->last_punch)))
        : $official_start_base;

      // أولاً: احسب التأخير من حد الدخول الأقصى
      $late_time_minutes  = ($first_punch_normalized > $max_entry)
        ? max(0, floor(($first_punch_normalized - $max_entry) / 60))
        : 0;

      // ثانياً: حدِّد نهاية الدوام وفقًا لوجود التأخير
      // - لو مافيه تأخير: نهاية الدوام = first_punch (الرسمية) + 9 ساعات
      // - لو فيه تأخير:   نهاية الدوام = last_punch + 9 ساعات
      if ($late_time_minutes > 0) {
        $official_end_normalized = $max_entry + $required_duration_seconds;       // last_punch + 9h
      } else {
        $official_end_normalized = $official_start_base + $required_duration_seconds; // first_punch + 9h
      }

      // الآن نحسب الخروج المبكر من النهاية الرسمية المحددة أعلاه
      ...
      /* ====== START: معالجة التأخير والخروج المبكر معًا بدون تغيير أي معادلات خصم لديك ====== */
      // ضمان عدم السالب
      $late_time_minutes  = max(0, $late_time_minutes);
      $early_exit_minutes = max(0, $early_exit_minutes);
      ...
      if ($late_time_minutes > 0) {
        if ($is_on_correction) {
          // تصحيح البصمة يلغي مخالفة التأخير
          ...
          // نفس معادلتك الأصلية لخصم التأخير
      ...
      if ($early_exit_minutes > 0) {
        if ($is_on_correction) {
          // تصحيح البصمة يلغي مخالفة الخروج المبكر
          ...
          // نفس معادلتك الأصلية لخصم الخروج المبكر
          $total_early_exit_minutes += $early_exit_minutes;
          $total_late_exit_deduction += ($early_exit_minutes * $daily_salary / 9 / 60);
        }
      }

      // 3) تحديد الرسالة والكلاس النهائي لليوم
      if ($late_time_minutes === 0 && $early_exit_minutes === 0) {
        $violation_class = 'on-time';
        $message = 'ملتزم بوقت العمل';
      } else {
        $class_tags = array_unique($class_tags);
        if (!empty($class_tags) && count(array_unique($class_tags)) === 1 && $class_tags[0] === 'fingerprint-correction') {
          $violation_class = 'fingerprint-correction';
        } else {
          $violation_class = implode(' ', $class_tags); // يسمح بوجود "late-arrival early-departure" معًا
        }
      ...
      /* ====== END: معالجة التأخير والخروج المبكر معًا ====== */
    }
  }
