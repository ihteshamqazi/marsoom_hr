...
// --- NEW, FAST CODE ---
$emp_code = $row->employee_id;
$vacations_data = $data_map['vacations'][$emp_code] ?? [];
$fingerprint_corrections_data = $data_map['corrections'][$emp_code] ?? [];
...
for ($date = $start_date; $date <= $end_date; $date = strtotime("+1 day", $date)) {
  ...
  if (!$is_on_vacation && !empty($fingerprint_corrections_data)) {
    foreach ($fingerprint_corrections_data as $corr) {
      if (empty($corr['start_date']) || empty($corr['end_date'])) continue;
  ...
  if ($is_on_vacation) {
    ...
    /* =================== استخراج بصمات اليوم (خروج من 13:00 إلى 21:00) =================== */
    $noon_time_limit    = strtotime($date_str . ' 12:00:00'); // لا يزال للدخول فقط
    $exit_start_limit   = strtotime($date_str . ' 13:00:00'); // بداية نافذة الخروج الصحيحة
    $exit_cutoff_limit  = strtotime($date_str . ' 21:00:00'); // أي خروج بعده يُتجاهل

    $earliest_in_before_noon = null;  // أبكر دخول قبل 12
    $latest_out_in_window    = null;  // أحدث خروج داخل [13:00..21:00]
    ...
    foreach ($attendance_data as $attendance) {
      if ($attendance->emp_code == $emp_code && $attendance->punch_date == $date_str) {
        ...
        // دخول: يُحسب فقط لو قبل 12:00 (بدون تغيير)
        ...
        // خروج: يُحسب فقط داخل نافذة [13:00 .. 21:00]
        // أي خروج بين 12:00 و 12:59:59 يتم تجاهله تلقائيًا
        if ($raw_last && $raw_last >= $exit_start_limit && $raw_last <= $exit_cutoff_limit) {
          $present_from_kept = true;
    ...
    $last_punch  = $latest_out_in_window;      // لا يحتسب 12:xx ولا ما بعد 21:00
    ...
    elseif (empty($first_punch) || empty($last_punch) || (($last_punch - $first_punch) < 60)) {
      // بصمة منفردة (طبقًا لنافذة الخروج الجديدة)
      $violation_class = 'early-departure';
      $message = 'بصمة منفردة';
      if (!$is_on_correction) {
        $single_punch_days++;
      ...
      // ========= منطق الحساب (دون تغيير) =========

      // --- NEW, FAST CODE ---
$rule_row = $data_map['rules'][$row->employee_id] ?? null;

      // تحويل working_hours إلى ساعات
      $working_hours = 9.0; // افتراضي لو مفقود
      ...
      $hours_divisor             = max(0.01, $working_hours);
      ...
      // البداية الرسمية وحد المرونة من القيود
      ...
      // التأخير: فقط إذا actual_in > max_entry
      $late_time_minutes  = ($first_punch_normalized > $max_entry)
        ? max(0, floor(($first_punch_normalized - $max_entry) / 60))
        : 0;

      // الخروج المبكر: نفس السياسة السابقة (لم تُغيّر)
      if ($late_time_minutes > 0) {
        $official_end_normalized = $max_entry + $required_duration_seconds;
        ...
        $official_end_normalized = $official_start_base + $required_duration_seconds;
