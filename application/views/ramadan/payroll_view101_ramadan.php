       ...
       // ==================================================================
        ...
        if ($rule_row && isset($rule_row->working_hours) && $rule_row->working_hours !== '') {
             ...
             if (strpos($wh_raw, ':') !== false) {
                 ...
                 $standard_contract_hours = ($parts[0] ?? 0) + (($parts[1] ?? 0)/60);
             } else { 
                 $standard_contract_hours = floatval($wh_raw); 
             }
        }
        
        // === RAMADAN FIX: 5 HOURS FOR BREASTFEEDING SHIFT, 6 FOR OTHERS ===
        // === RAMADAN FIX: 5 HOURS FOR BREASTFEEDING SHIFT, 6 FOR OTHERS ===
        // Strictly check the database rule row
        $is_breastfeeding_shift = ($rule_row && isset($rule_row->working_hours) && (float)$rule_row->working_hours == 8.0);
        
        if ($is_breastfeeding_shift) {
            $default_working_hours = 5.0; 
        } else {
            $default_working_hours = 6.0;
        }
...
>
   ...
   onclick="openAttendancePopup('<?php echo site_url('users1/employee_daily_log_ramadan/' . $row->employee_id . '/' . $id); ?>')"
...
function updateBankExportLink() {
    ...
    var baseUrl = "<?php echo site_url('ramadan/export_bank_payroll_csv/'); ?>" + sheetId;
...
$('#companyFilter').on('change', function() {
    ...
    var baseExportUrl = "<?php echo site_url('ramadan/export_payroll_sheet_csv/' . $id); ?>";
...
// ==================================================================
        ...
        $('.dataTables-example tbody').on('click', '.view-details-link', function(e) {
            ...
            $.ajax({
                url: "<?php echo site_url('ramadan/get_ramadan_violation_details'); ?>",
                type: 'POST',
                data: {
                    emp_id: empId,
                    sheet_id: sheetId,
...
const filteredPayrollBatch = fullPayrollBatch.filter(empData => {
    ...
    return selectedCompanyCodes.includes(String(empData.n14));
                    ...
                    $.ajax({
                        url: "<?php echo site_url('users1/process_payroll'); ?>", // <-- Changed to Ramadan                        
                        type: 'POST',
                        data: {
