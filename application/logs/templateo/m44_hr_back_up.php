  ...
  if ($is_on_vacation) {
    ...
    foreach ($attendance_data as $attendance) {
      if ($attendance->emp_code == $emp_code && $attendance->punch_date == $date_str) {
        ...
        // نفس منطقك السابق: تحديد أول وآخر بصمة حسب الظهر
        $noon_time_limit = strtotime($date_str . ' 12:00:00');
        $first_punch = ($raw_first && $raw_first < $noon_time_limit) ? $raw_first : null;
        $last_punch  = ($raw_last  && $raw_last  >= $noon_time_limit) ? $raw_last  : null;

        // حاضر إذا لديه أي بصمة خام
        $is_present = (bool)($raw_first || $raw_last);
        break;
      }
    }
    ...
    if (!$is_present) {
      ...
      // تصحيح البصمة يلغي خصم الغياب
      if (!$is_on_correction) {
        $absence_days++; 
        $total_absence_deduction += $daily_salary; 
      } else {
        $violation_class = 'fingerprint-correction'; // NEW
        $message .= ' (مستثنى بتصحيح بصمة: ' . $correction_type . ')'; // NEW
      }
    ...
    elseif (empty($first_punch) || empty($last_punch) || (($last_punch - $first_punch) < 60)) {
      ...
      // تصحيح البصمة يلغي خصم البصمة المنفردة
      if (!$is_on_correction) {
        $single_punch_days++;
      ...
      // ===================== منطق القيود كما طلبت =====================
      $required_duration_seconds = 9 * 3600;

      $sheet_id = $this->uri->segment(3,0);
      // نقرأ first_punch (بداية رسمية) و last_punch (حد الدخول الأقصى)
      $rule_row = $this->db->select('first_punch, last_punch')
      ...
      // 1) بداية الدوام الرسمية = first_punch (fallback: last_punch ثم 08:00)
      if (!empty($rule_row->first_punch)) {
        $official_start_normalized = strtotime(date('Y-m-d H:i:00', strtotime($date_str.' '.$rule_row->first_punch)));
      } elseif (!empty($rule_row->last_punch)) {
        $official_start_normalized = strtotime(date('Y-m-d H:i:00', strtotime($date_str.' '.$rule_row->last_punch)));
      } else {
        $official_start_normalized = strtotime($date_str.' 08:00:00');
      }

      // 2) حد الدخول الأقصى (الساعات المرنة) = last_punch (fallback: بداية الدوام)
      $max_entry_normalized = !empty($rule_row->last_punch)
        ? strtotime(date('Y-m-d H:i:00', strtotime($date_str.' '.$rule_row->last_punch)))
        : $official_start_normalized;

      // 3) نهاية الدوام الرسمية = بداية الدوام + 9 ساعات (ثابتة)
      $official_end_normalized = $official_start_normalized + $required_duration_seconds;

      // --------- حساب المخالفتين بحسب القواعد ---------
      // التأخير: فقط إذا actual_in > max_entry، وفرق التأخير من max_entry
      $late_time_minutes = ($first_punch_normalized > $max_entry_normalized)
        ? max(0, floor(($first_punch_normalized - $max_entry_normalized) / 60))
        : 0;

      // الخروج المبكر: إذا actual_out < official_end، والفرق من official_end
      $early_exit_minutes = ($last_punch_normalized < $official_end_normalized)
        ? max(0, floor(($official_end_normalized - $last_punch_normalized) / 60))
        : 0;

      // ---------------- عرض وتراكم المخالفات معًا ----------------
      $messages_today = [];
      $class_tags     = [];
      ...
      if ($late_time_minutes > 0) {
        if ($is_on_correction) {
          $messages_today[] = 'دخول متأخر: ' . $late_time_minutes . ' دقيقة (مستثنى بتصحيح بصمة: ' . $correction_type . ')';
          $class_tags[] = 'fingerprint-correction';
        } else {
          $messages_today[] = 'دخول متأخر: ' . $late_time_minutes . ' دقيقة';
          $class_tags[] = 'late-arrival';
          $total_late_minutes += $late_time_minutes; 
          $total_late_exit_deduction += ($late_time_minutes * $daily_salary / 9 / 60);
        }
      }
      ...
      if ($early_exit_minutes > 0) {
        if ($is_on_correction) {
          $messages_today[] = 'خروج مبكر: ' . $early_exit_minutes . ' دقيقة (مستثنى بتصحيح بصمة: ' . $correction_type . ')';
          $class_tags[] = 'fingerprint-correction';
        } else {
          $messages_today[] = 'خروج مبكر: ' . $early_exit_minutes . ' دقيقة';
      ...
      if ($late_time_minutes === 0 && $early_exit_minutes === 0) {
        ...
        // إن وُجدت مخالفتان، تُعرضان معًا
