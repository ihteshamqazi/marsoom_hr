...
<div class="main-container container-fluid">
    <div class="text-center"><h1 class="page-title">Work Restrictions</h1></div>
    <div class="card table-card">
        <div class="card-body">
            <table class="table table-striped table-bordered dataTables-example w-100">
                <thead><tr><th>#</th><th>Emp ID</th><th>Emp Name</th><th>Management</th><th>Company</th><th>Working Hours</th><th>Effective Date</th><th>Actions</th></tr></thead>
                <tbody></tbody>
            </table>
        </div>
    </div>
            ...
            <div class="modal-body">
                <form id="genericForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3"><label>Management</label><input type="text" name="management" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Company</label><input type="text" name="company" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Sheet ID</label><input type="text" name="sheet_id" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Working Hours</label><input type="number" step="any" name="working_hours" class="form-control"></div>
                        
                        <div class="col-md-4 mb-3"><label>First Punch</label><input type="datetime-local" step="1" name="first_punch" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Last Punch</label><input type="datetime-local" step="1" name="last_punch" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Max Departure Date</label><input type="datetime-local" step="1" name="maximum_departure_date" class="form-control"></div>
                        
                        <div class="col-md-12 mb-3"><label>Effective Date</label><input type="date" name="effective_date" class="form-control"></div>
                    </div>
                </form>
            </div>
            <div class="modal-footer"><button class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button type="submit" form="genericForm" class="btn btn-primary">Save</button></div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        const TABLE_NAME = 'work_restrictions';
        
        var dt = $('.dataTables-example').DataTable({
            pageLength: 20, processing: true, serverSide: true, order: [],
            ajax: { url: "<?php echo site_url('emp_management/dt_work_restrictions'); ?>", type: "POST", data: function(d) { d[csrfName] = csrfHash; } },
        ...
        $('.dataTables-example').on('click', '.edit-btn', function() {
            ...
            $.post("<?php echo site_url('emp_management/generic_action'); ?>", {action:'get', table:TABLE_NAME, id: id, [csrfName]: csrfHash}, function(res) {
                ...
                $.each(j.data, function(k, v) {
                    if((k === 'first_punch' || k === 'last_punch' || k === 'maximum_departure_date') && v !== null) { $('[name="'+k+'"]').val(v.replace(' ', 'T')); } 
                    else { $('[name="'+k+'"]').val(v); }
                }); 
