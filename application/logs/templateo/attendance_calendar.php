            ...
            <table class="calendar w-100">
                ...
                <?php
                    ...
                    for ($d=1;$d<=$daysInMonth;$d++){
                        ...
                        $isAbsent  = !$has && empty($evs) && !$isWeekend && !$isHoliday && ($date <= $today);
                        
                        $classes = 'day-cell';
                        if ($has) $classes .= ' has-attendance';
                        if ($date === $today) $classes .= ' today';
                        if ($isAbsent) $classes .= ' absent';
                        if ($isHoliday) $classes .= ' public-holiday';
                        if ($isWeekend && !$has && empty($evs) && !$isHoliday) $classes .= ' weekend';
...
<script>
    ...
    // Helper function to render events for the details card
    // In your <script> tag at the bottom of the file
...
function renderEvents(date) {
    ...
    if (eventsForDay.length > 0) {
        ...
        eventsForDay.forEach(ev => {
            ...
            // ✅ FIX IS HERE: We convert ev.status to a number first
            let statusNum = Number(ev.status);
            let badge;

            if (statusNum === 2) {
                badge = '<span class="badge bg-success">معتمد</span>';
            } else if (statusNum === 3) {
                badge = '<span class="badge bg-danger">مرفوض</span>';
            } else if (statusNum === -2) {
                badge = '<span class="badge bg-dark">ملغى من الموارد البشرية</span>';
            } else {
                badge = '<span class="badge bg-warning">بالإنتظار</span>';
            }
            
            html += `<li class="mb-1">${ev.title} ${badge}</li>`;
        });
        html += '</ul>';
    }
    ...
    // Helper function to render violations for the details card
    function renderViolations(date) {
        const violationsForDay = VIOLATIONS[date] || [];
        let html = '';
        if (violationsForDay.length > 0) {
            html += '<h6 class="mt-2"><strong>المخالفات:</strong></h6><ul class="list-unstyled mb-0">';
    ...
    // Main logic for handling clicks on calendar days
