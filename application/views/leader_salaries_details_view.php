...
<head>
  ...
  <title><?= html_escape($title ?? 'تفاصيل القيادي') ?></title>
  ...
  <style>
    ...
    .title-box h1{font-family:'El Messiri',serif;font-size:1.6rem;font-weight:900;margin:0;background:linear-gradient(135deg,#fff,#ffd166);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
...
<div class="wrap">
  ...
  <div class="header-nav" data-aos="fade-down" data-aos-duration="800">
    <div class="title-box">
      <h1>تفاصيل القيادي - التابعين </h1>
      <p>
        القيادي رقم: <b><?= html_escape($manager_id ?? '') ?></b>
        <?php if (!empty($manager_info['subscriber_name'])): ?>
          — الاسم: <b><?= html_escape($manager_info['subscriber_name']) ?></b>
        <?php endif; ?>
      </p>
    </div>
    <div class="d-flex gap-2 flex-wrap no-print">
      <a class="btn-marsom" href="<?= site_url('LeaderSalariesReport'); ?>"><i class="fa-solid fa-arrow-right"></i> رجوع</a>
      <button class="btn-marsom primary" onclick="window.print()"><i class="fa-solid fa-print"></i> طباعة</button>
      <a class="btn-marsom report" href="<?= site_url('LeaderSalariesReport/export_n3/'.rawurlencode($manager_id)).( $_GET ? ('?'.http_build_query($_GET)) : '' ) ?>">
  ...
  <?php if (!$has_n3): ?>
    <div class="section" data-aos="fade-up" data-aos-delay="80">
      <div class="empty">لا يوجد موظفين تابعين لادارته</div>
    </div>
  <?php else: ?>

    <?php foreach (($groups_n3 ?? []) as $gi => $g): ?>
      <?php
        $stats = $g['stats'] ?? [];
        $rows  = $g['rows'] ?? [];
        $cnt   = (int)($stats['cnt'] ?? 0);
      ...
      <div class="section" data-aos="fade-up" data-aos-delay="<?= 120 + ($gi*60) ?>">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
          <div class="company-badge"><i class="fa-solid fa-users"></i> <?= html_escape($g['label'] ?? '') ?></div>
        ...
        <?php else: ?>
              ...
              <table class="table table-hover align-middle">
                <thead>
                  <tr>
                    ...
                    <th>إجمالي الراتب</th>
                    <th class="no-print" style="width:220px">التفاصيل</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                      ...
                      <td class="no-print">
                        <a class="btn-marsom btn-details-black"
                           href="<?= site_url('LeaderSalariesReport/sub_details/'.rawurlencode($r['employee_id'] ?? '')).$qs ?>">
                          <i class="fa-solid fa-circle-nodes"></i> تفاصيل
                        </a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
          ...
          <div class="d-md-none">
            <?php $i=1; foreach($rows as $r): ?>
              <?php $cid = 'mN3_'.$g['n13'].'_'.$i.'_'.$r['employee_id']; ?>
              <div class="m-card">
                <div class="m-head">
                  <div>
                    <div class="m-name"><?= html_escape($r['subscriber_name'] ?? '') ?></div>
                    <div class="m-sub">
                      <i class="fa-solid fa-id-badge"></i> <?= html_escape($r['employee_id'] ?? '') ?>
                    </div>
                  </div>
                ...
                <div class="m-actions">
                  <button class="btn-marsom btn-details-black" type="button"
