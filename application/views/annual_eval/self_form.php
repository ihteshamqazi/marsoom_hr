...
<head>
  ...
  <title><?= html_escape($title ?? 'تقييم الأداء السنوي - تقييم ذاتي') ?></title>
  ...
  <style>
    ...
    .warn{
      background:#fff6d6;border:1px solid #ffe3a3;border-radius:14px;
      padding:12px 14px;color:#7a5b00
    }
    ...
    .kpi .box{background:#fafbff;border:1px solid #e7ebf0;border-radius:14px;padding:10px 12px}
    ...
    .desc{color:#666;font-size:12.5px;line-height:1.6;margin-top:6px}
    ...
    .btn-marsom{background:var(--marsom-blue);border-color:var(--marsom-blue);font-weight:800}
    .btn-marsom:hover{background:#001733;border-color:#001733}
...
$total_max = (float)($total_max ?? 120);
  ...
  $self = (isset($self) && is_array($self)) ? $self : [];
...
$reasons   = (isset($reasons) && is_array($reasons)) ? $reasons : [];
  ...
  $get = function($k, $default=0) use ($self){
    return isset($self[$k]) ? $self[$k] : $default;
  };
?>
...
<div class="shell">
  ...
  <div class="topbar">
    <div class="brand">
      ...
      <div>
        <p class="ttl mb-1">تقييم الأداء السنوي - تقييم ذاتي</p>
        <div class="meta">
          <b><?= html_escape($emp['emp_name'] ?? '') ?></b>
          <span class="text-muted">|</span>
          رقم وظيفي: <?= html_escape($emp['emp_no'] ?? '') ?>
          <span class="text-muted">|</span>
    ...
    <div class="meta text-start">
      <span class="pill pill-year">العام: 2025</span>
      <?php if (!empty($self['total_score'])): ?>
        <div class="mt-2">آخر إجمالي: <b><?= number_format((float)$self['total_score'],2,'.','') ?></b> / <?= number_format($total_max,2,'.','') ?></div>
        <div class="text-muted">التصنيف: <b><?= html_escape($self['grade_label'] ?? '-') ?></b></div>
      <?php else: ?>
        <div class="mt-2 text-muted">لم يتم حفظ تقييم سابق لهذه السنة.</div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="cardx">
    <!-- <div class="warn">
      <b>تنبيه الخصوصية:</b> تقييمك الذاتي لا يطلع عليه أي شخص داخل النظام باستثناء <b>مدير النظام</b> ضمن شاشة الإدارة.
    </div> -->
    ...
    <div class="kpi">
      ...
      <div class="box">
        <div class="label">الانضباط (من CSV)</div>
        <div class="value"><?= number_format($discipline,2,'.','') ?> / 20</div>
      </div>
      <div class="box">
        <div class="label">قاعدة الدورات</div>
        <div class="value"><?= number_format($courses_base,2,'.','') ?> / 20</div>
      </div>
      <div class="box">
  ...
  <div class="cardx">
    <div class="alert alert-success mb-2">
      شكراً لك، تم التقييم مسبقاً ولا يمكن إدخال التقييم مرة أخرى.
    </div>

    <?php if ($show_result): ?>
      <div class="alert alert-info mb-0">
        تم اكتمال التقييم الذاتي وتقييم المسؤول المباشر، ويمكنك الآن الاطلاع على التقرير النهائي.
        <div class="mt-2">
          <a class="btn btn-outline-marsom"
             target="_blank"
             href="<?= site_url('AnnualEvaluation/print_a4/' . rawurlencode($emp['emp_no'] ?? '') . '?year=' . (int)$year) ?>">
             عرض التقرير النهائي
          </a>
        </div>
      </div>
    <?php else: ?>
      <div class="alert alert-warning mb-0">
        تم حفظ تقييمك الذاتي بنجاح، وسيتم إظهار النتيجة بعد اكتمال تقييم المسؤول المباشر.
      </div>
    <?php endif; ?>
  </div>
<?php endif; ?>
    ...
    <?php if (!$has_self): ?>
<form method="post" action="<?= rtrim(base_url(),'/') . '/AnnualEvaluation/save' ?>" id="evalForm">

      <input type="hidden" name="year" value="<?= (int)($year ?? date('Y')) ?>">

      <!-- Criteria section -->
      <div class="section-title">معايير التقييم وإدخال الدرجات</div>
  ...
  if ($k === 'discipline_score') {
    ...
    // في self_dev/exceptional: لا ينقص عن base
...
<div class="crit"
  ...
  <?php if ($readonly): ?>
    <div class="mini">هذه الدرجة تُسحب تلقائيًا من ملف الانضباط ولا يمكن تعديلها.</div>
  <?php else: ?>
    ...
    <div class="mt-2">
      <?php foreach ($parts as $p): ?>
        <?php
          ...
          // سبب البند (يظهر فقط إذا أقل من max)
        ...
        <div class="border rounded-3 p-2 mb-2 part-row" data-part-max="<?= (float)$pmax ?>">
          <div class="d-flex justify-content-between align-items-center gap-2 flex-wrap">
            <div class="fw-bold"><?= html_escape($p['name']) ?> <span class="text-muted">(<?= (int)$pmax ?>)</span></div>
             ...
             <?php $note_rule = (string)($p['note_rule'] ?? 'required_on_less'); ?>
<input
  type="number"
...
>
          ...
          <div class="mt-2 part-reason-wrap" style="display:none;">
            <label class="mini fw-bold d-block mb-1">سبب (إجباري عند نقصان الدرجة)</label>
            <textarea
              class="form-control part-reason"
              name="reasons[<?= html_escape($k) ?>][<?= html_escape($pk) ?>]"
              rows="2"
              placeholder="اكتب سبب وضع درجة أقل من <?= (int)$pmax ?>..."
  ...
  <?php endif; ?>
      ...
      <!-- Notes -->
      <div class="section-title">ملاحظات (اختياري)</div>
      <textarea class="form-control" name="notes" rows="5"><?= html_escape($get('notes','')) ?></textarea>

      <!-- Actions -->
      <div class="actions">
        <button class="btn btn-marsom px-4" type="submit">حفظ التقييم</button>
        <a class="btn btn-outline-marsom"
           href="<?= site_url('AnnualEvaluation/print_a4/'.rawurlencode($emp['emp_no'] ?? '').'?year='.(int)($year ?? date('Y'))) ?>"
           target="_blank">معاينة الطباعة</a>
        <a class="btn btn-outline-secondary" href="<?= current_url().'?year='.(int)($year ?? date('Y')) ?>">تحديث</a>
      </div>
        ...
        <div class="d-flex align-items-start gap-3">
          ...
          <div>
            ...
            <div style="line-height:2;color:#333;font-size:15px;">
              أقر وأتعهد بأن جميع بيانات التقييم المدخلة صحيحة، وأن لدي ما يثبت البنود التي تتطلب إثباتًا، وأوافق على اعتماد هذا التقييم بشكل نهائي، مع علمي بأنه لن يكون بالإمكان إعادة التقييم مرة أخرى بعد الحفظ.
            </div>
          </div>
        </div>
...
// نوع إلزام الملاحظة من لوحة التحكم
if (partRule === 'optional') {
  ...
  // ✅ الحد الأدنى فقط للمعايير المرتبطة بالدورات
  if (reasonMode === 'positive_proof') {
  ...
  if (totalEl) {
    totalEl.value = total.toFixed(2) + ' / ' + critMax;
  }

  // ✅ سبب المعيار التراكمي
  ...
  if (critReason){
    if (reasonsLines.length){
      ...
      critReason.value = '';
      critReason.required = false; // ✅ هذه أهم نقطة
      critReason.classList.remove('is-invalid');
    }
  }

  // خزّن هل هذا المعيار يحتاج سببًا حاليًا
...
function recalcTotal(){
  ...
  const disInput = document.querySelector('.crit[data-crit="discipline_score"] .crit-total');
  if (disInput){
    total += num(String(disInput.value).split('/')[0]);
  }

  document.querySelectorAll('.crit-total').forEach(el => {
    const key = el.closest('.crit')?.dataset?.crit;
    if (!key || key === 'discipline_score') return;
