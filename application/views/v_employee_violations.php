...
<head>
    <meta charset="UTF-8">
    <title>Employee Violations</title>
...
<div class="main-container container-fluid">
    <div class="text-center"><h1 class="page-title">Employee Violations</h1></div>
    <div class="card table-card">
        <div class="card-body">
            <table class="table table-striped table-bordered dataTables-example w-100">
                <thead><tr><th>#</th><th>Emp ID</th><th>Emp Name</th><th>Department</th><th>Amount</th><th>Violation Date</th><th>Actions</th></tr></thead>
                <tbody></tbody>
            </table>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="genericForm">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3"><label>Employee ID</label><input type="text" name="employee_id" class="form-control"></div>
                        <div class="col-md-6 mb-3"><label>Emp Name</label><input type="text" name="emp_name" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Department</label><input type="text" name="department" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Amount</label><input type="number" step="any" name="amount" class="form-control"></div>
                        <div class="col-md-4 mb-3"><label>Supervisor Name</label><input type="text" name="supervisor_name" class="form-control"></div>
                        <div class="col-md-6 mb-3"><label>Violation Date</label><input type="date" name="violation_date" class="form-control"></div>
                        <div class="col-md-6 mb-3"><label>Created By</label><input type="text" name="created_by" class="form-control"></div>
                        <div class="col-md-12 mb-3"><label>Created At</label><input type="datetime-local" step="1" name="created_at" class="form-control"></div>
                        <div class="col-md-6 mb-3"><label>HR Note</label><textarea name="hr_note" class="form-control" rows="3"></textarea></div>
                        <div class="col-md-6 mb-3"><label>Employee Feedback</label><textarea name="employee_feedback" class="form-control" rows="3"></textarea></div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        const TABLE_NAME = 'employee_violations';
        
        var dt = $('.dataTables-example').DataTable({
            pageLength: 20, processing: true, serverSide: true, order: [],
            ajax: { url: "<?php echo site_url('emp_management/dt_employee_violations'); ?>", type: "POST", data: function(d) { d[csrfName] = csrfHash; } },
        ...
        $('.dataTables-example').on('click', '.edit-btn', function() {
            ...
            $.post("<?php echo site_url('emp_management/generic_action'); ?>", {action:'get', table:TABLE_NAME, id: id, [csrfName]: csrfHash}, function(res) {
                ...
                $.each(j.data, function(k, v) {
                    if(k === 'created_at' && v !== null) { $('[name="'+k+'"]').val(v.replace(' ', 'T')); } 
                    else { $('[name="'+k+'"]').val(v); }
                }); 
