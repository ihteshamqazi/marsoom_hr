...
<div style="position: fixed; top: 1rem; right: 1rem; z-index: 1000;">
    <button onclick="testSave()" class="btn btn-warning">Test Save</button>
</div>

<script>
function testSave() {
    console.log('Testing save function...');
    const testHtml = '<div>Test HTML content ' + new Date().toLocaleString() + '</div>';
    
    fetch('<?php echo site_url("users1/test_save_endpoint"); ?>', {
        method: 'POST',
        body: JSON.stringify({
            letter_html_content: testHtml,
            letter_slug: 'test',
            employee_id: 'test',
...
<script>
    $(document).ready(function() {
        ...
        $('.letter-card').on('click', function() {
            ...
            $('#employeeDropdown').val(null).trigger('change').empty();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success' && response.employees.length > 0) {
                        $('#employeeDropdown').append(new Option('-- اختر الموظف --', '', true, true));
                        $.each(response.employees, function(i, employee) {
                            // Store order_id as a data attribute on the option
                            var option = new Option(employee.name + ' (طلب #' + employee.order_id + ')', employee.id, false, false);
                            $(option).data('order-id', employee.order_id);
                            $('#employeeDropdown').append(option);
                        });
                    } else {
                        $('#employeeDropdown').append(new Option('لا يوجد طلبات خطابات حالياً', '', true, true));
                    }
                    $('#employeeDropdown').trigger('change');
                },
                error: function() { $('#employeeDropdown').append(new Option('خطأ في تحميل البيانات', '', true, true)).trigger('change'); }
            });
    ...
    const selectedOption = $('#employeeDropdown').find(':selected');
    const employeeId = selectedOption.val();
    const orderId = selectedOption.data('order-id');
    ...
    // Updated URL to include order_id as third parameter
    const url = `<?= site_url('users1/generate_letter') ?>/${selectedLetterSlug}/${employeeId}/${orderId}`;
